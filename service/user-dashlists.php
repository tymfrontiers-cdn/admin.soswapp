...
<html lang="en" dir="ltr" manifest="<?php echo WHOST; ?>/site.webmanifest">
  ...
  <body>
    ...
    <section id="main-content">
      ...
      id="delete-form"
      method="post"
      action="/app/tymfrontiers-cdn/admin.soswapp/src/DeleteDashlist.php"
      data-validate="false"
      onsubmit="sos.form.submit(this,checkDelete);return false;"
      >
      <input type="hidden" name="form" value="dashlist-delete-form">
      <input type="hidden" name="CSRF_token" value="<?php echo $session->createCSRFtoken("dashlist-delete-form");?>">
        ...
        <br class="c-f">
          <div class="grid-8-tablet center-tablet">
            <form
              ...
              method="post"
              action="/app/tymfrontiers-cdn/admin.soswapp/src/FetchDashlist.php"
              data-validate="false"
              onsubmit="sos.form.submit(this,doFetch);return false;"
              >
              <input type="hidden" name="form" value="dashlist-query-form">
              <input type="hidden" name="CSRF_token" value="<?php echo $session->createCSRFtoken("dashlist-query-form");?>">
    ...
    <script type="text/javascript">
      ...
      function doFetch(data){
        if( data && data.status == "0.0" && data.dashlists.length > 0){
          ...
          $('#record-list').listData( data.dashlists );
          removeAlert();
        }else{
          $('#record-list').html('');
        }
      ...
      function doDelete(id){
        if( confirm("Are you sure you want to delete this dashlist?") ){
