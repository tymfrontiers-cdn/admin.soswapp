...
\require_login(true);
\check_access("/work-domains", true, "project-admin");
?>
<!DOCTYPE html>
<html lang="en" dir="ltr" manifest="/site.webmanifest">
  ...
  <body>
    ...
    <section id="main-content">
      ...
      id="delete-form"
      method="post"
      action="/app/tymfrontiers-cdn/admin.soswapp/src/DeleteWorkDomain.php"
      data-validate="false"
      onsubmit="sos.form.submit(this,checkDelete);return false;"
      >
      <input type="hidden" name="form" value="domain-delete-form">
      <input type="hidden" name="CSRF_token" value="<?php echo $session->createCSRFtoken("domain-delete-form");?>">
        ...
        <br class="c-f">
          <div class="grid-8-tablet center-tablet">
            <form
              ...
              method="post"
              action="/app/tymfrontiers-cdn/admin.soswapp/src/FetchWorkDomain.php"
              data-validate="false"
    ...
    <script type="text/javascript">
      ...
      function doFetch(data){
        if( data && data.status == "0.0" && data.domains.length > 0){
          ...
          $('#record-list').listData( data.domains );
          removeAlert();
        }else{
          $('#record-list').html('');
        }
