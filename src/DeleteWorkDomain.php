...
if( !$domain ){
  echo \json_encode([
    "status" => "3.1",
    "errors" => ["[domain] ({$params['name']}) not found."],
...
// delete user path access setting
$db->query("DELETE FROM `".MYSQL_ADMIN_DB."`.path_access
            WHERE path_name IN (
              SELECT name
              FROM `".MYSQL_ADMIN_DB."`.work_path
              WHERE domain='{$db->escapeValue($params['name'])}'
            )");
// delete domain work group
$db->query("DELETE FROM ".MYSQL_ADMIN_DB.".work_path WHERE domain='{$db->escapeValue($params['name'])}'");
